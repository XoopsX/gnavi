 ...
 * -------------------------------------------------------------
 * Type:     function
 * Name:     d3forum_comment_topicscount
 * Version:  1.0
 * Date:     Oct 16, 2006
 * Author:   GIJOE
 * Purpose:  
 * Input:    
 * 
 * Examples: {d3forum_comment_topicscount dirname=d3forum forum_id=1}
 * -------------------------------------------------------------
 */
function smarty_function_d3forum_comment_topicscount($params, &$smarty)
{
	...
	if( ! preg_match( '/^[0-9a-zA-Z_-]+$/' , $dirname ) || $forum_id <= 0 || ! file_exists( XOOPS_TRUST_PATH.'/modules/d3forum/include/comment_functions.php' ) ) {
		...
		d3forum_display_comment_topicscount( $dirname , $forum_id , $params , 'topic' ) ;
	}
}
