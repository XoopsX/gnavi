...
eval( ' function xoops_module_uninstall_'.$mydirname.'( $module ) { return gnavi_onuninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'gnavi_onuninstall_base' ) ) {

function gnavi_onuninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleUninstall.' . ucfirst($mydirname) . '.Success' , 'gnavi_message_append_onuninstall' ) ;
		$ret = array() ;
	} else {
...
function gnavi_message_append_onuninstall( &$module_obj , &$log )
{
